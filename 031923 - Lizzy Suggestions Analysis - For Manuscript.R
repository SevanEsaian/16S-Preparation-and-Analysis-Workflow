...
#Stacked Barplots ---------
relative.abundances <- read.csv("RA for Stacked Barplot.csv")
orders <- read.csv("species_coords.csv")
just.orders <- orders[, c(1,5)]
merged.ra.orders <- merge(just.orders, relative.abundances, by = "AmpliconSequenceVariant")
merged.ra.orders.only <- merged.ra.orders[,c(2:88)]

summed_orders_per_UI <- merged.ra.orders.only %>%
  group_by(order) %>%
  summarise(across(everything(), sum, na.rm=TRUE))

order.ra.bplot <- read.csv("stacked barplots orders.csv")
#Juvenile Surface
order.ra.bplot.juv.surf <- subset(order.ra.bplot, Age=="Juvenile" & Depth =="Surface")
custom_order <- c("<1% Relative Abundance", 
                  "Unknown", 
                  "Alteromodales",
                  "Caulobacterales",
                  "Cellvibrioles",
                  ...
                  "Verrucomicrobiales",
                  "Vibrioles")
order.ra.bplot.juv.surf$Order <- factor(order.ra.bplot.juv.surf$Order, levels=custom_order)
fill_colors <- c("<1% Relative Abundance" = "black",
                 "Unknown" = "gray",
                 "Chitinophagales" = "pink",
                 "Flavobacteriales" = "magenta",
                 "Synechococcales" = "green",
                 "Pirellulales" = "yellow",
                 "Caulobacterales" = "salmon",
                 "Rhizobiales" = "red",
                 "Rhodobacterales" = "firebrick",
                 "Alteromodales" = "skyblue",
                 "Cellvibrioles" = "dodgerblue",
                 "Granulosicoccales" = "blue",
                 "Oceanospirillales" = "royalblue",
                 "Pseudomodales" = "skyblue4",
                 "Thiotrichales" = "darkblue",
                 "Vibrioles" = "cyan",
                 "Verrucomicrobiales" = "darkorange")
juv.surf <- ggplot(order.ra.bplot.juv.surf, aes(fill=Order, y=Relative.Abundance, x=BladeSample)) +
  geom_bar(position="stack", stat="identity", color="white") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1)) +
  scale_fill_manual(values=fill_colors) +
  ggtitle("Juvenile Surface") +
  theme(plot.title=element_text(hjust=0.5)) +
  theme(axis.title.x = element_blank(),
        legend.position="none") + ylab("% Relative Abundance")

#Mature Surface
order.ra.bplot.mat.surf <- subset(order.ra.bplot, Age=="Mature" & Depth =="Surface")
order.ra.bplot.mat.surf$Order <- factor(order.ra.bplot.mat.surf$Order, levels=custom_order)
mat.surf <- ggplot(order.ra.bplot.mat.surf, aes(fill=Order, y=Relative.Abundance, x=BladeSample)) +
  geom_bar(position="stack", stat="identity", color="white") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1)) +
  scale_fill_manual(values=fill_colors) +
  ggtitle("Mature Surface") +
  theme(plot.title=element_text(hjust=0.5)) + 
  theme(axis.title.x = element_blank(),
...
#Juvenile Middle
order.ra.bplot.juv.mid <- subset(order.ra.bplot, Age=="Juvenile" & Depth =="Middle")
order.ra.bplot.juv.mid$Order <- factor(order.ra.bplot.juv.mid$Order, levels=custom_order)
juv.mid <- ggplot(order.ra.bplot.juv.mid, aes(fill=Order, y=Relative.Abundance, x=BladeSample)) +
  geom_bar(position="stack", stat="identity", color="white") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1)) +
  scale_fill_manual(values=fill_colors) +
  ggtitle("Juvenile Middle") +
  theme(plot.title=element_text(hjust=0.5)) +
  theme(axis.title.x = element_blank(),
        legend.position="none") + ylab("% Relative Abundance")

#Mature Middle
order.ra.bplot.mat.mid <- subset(order.ra.bplot, Age=="Mature" & Depth =="Middle")
order.ra.bplot.mat.mid$Order <- factor(order.ra.bplot.mat.mid$Order, levels=custom_order)
mat.mid <- ggplot(order.ra.bplot.mat.mid, aes(fill=Order, y=Relative.Abundance, x=BladeSample)) +
  geom_bar(position="stack", stat="identity", color="white") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1)) +
  scale_fill_manual(values=fill_colors) +
  ggtitle("Mature Middle") +
  theme(plot.title=element_text(hjust=0.5)) +
  theme(plot.title=element_text(hjust=0.5)) + 
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
...
#Juvenile Bottom
order.ra.bplot.juv.bot <- subset(order.ra.bplot, Age=="Juvenile" & Depth =="Bottom")
order.ra.bplot.juv.bot$Order <- factor(order.ra.bplot.juv.bot$Order, levels=custom_order)
juv.bot <- ggplot(order.ra.bplot.juv.bot, aes(fill=Order, y=Relative.Abundance, x=BladeSample)) +
  geom_bar(position="stack", stat="identity", color="white") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1)) +
  scale_fill_manual(values=fill_colors) +
  ggtitle("Juvenile Bottom") +
  theme(plot.title=element_text(hjust=0.5)) +
  theme(legend.position="none") + ylab("% Relative Abundance") + xlab("Blade Samples")

#Mature Bottom
order.ra.bplot.mat.bot <- subset(order.ra.bplot, Age=="Mature" & Depth =="Bottom")
order.ra.bplot.mat.bot$Order <- factor(order.ra.bplot.mat.bot$Order, levels=custom_order)
mat.bot <- ggplot(order.ra.bplot.mat.bot, aes(fill=Order, y=Relative.Abundance, x=BladeSample)) +
  geom_bar(position="stack", stat="identity", color="white") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1)) +
  scale_fill_manual(values=fill_colors) +
  ggtitle("Mature Bottom") +
  theme(plot.title=element_text(hjust=0.5)) +
  xlab ("Blade Samples")

juv.surf + mat.surf + juv.mid + mat.mid + juv.bot + mat.bot + plot_layout(ncol=2)
